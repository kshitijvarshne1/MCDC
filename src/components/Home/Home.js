...
function Home() {
    ...
    const handleSubmit = () => {
        ...
        if (!regex.test(inputText)) {
            notification("error", "Input text should contain only A-Z, (, ), |, and & characters");
            return;
        }
        if (!unique(inputText)) {
            notification("error", "Input text should contain only unique characters in case of characters");
            return;
        }
        if (!criteria) {
        ...
        <div >
            ...
            <div className='fullDiv'>
                <div>
                    ...
                    <Input
                        value={inputText}
                        onChange={(e) => { setInputText(e.target.value); setSolution(false) }}
                        placeholder="Enter character or symbols ex: A | B"
                    />
                    <div className='infoDiv'>
                        <InfoCircleOutlined />
                        <p className='infoP' >Use '|' for logical OR and '&' for logical AND</p>
                    </div>
                    <div className='selectCriteriaDiv'>
                        <Radio.Group onChange={onChange} value={criteria}>
                            <Space direction="vertical">
                                ...
                                <Radio value={"combinational"}>Combinational Coverage</Radio>
